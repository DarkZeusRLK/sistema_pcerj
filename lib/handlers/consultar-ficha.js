...
module.exports = async (req, res) => {
  ...
  try {
    ...
    todosRegistros.forEach((msg) => {
      somaMultas += extrairValorMultaDaMensagem(msg);
      totalInafiancaveis += contarInafiancaveisDaMensagem(
        msg,
        listaKeywordsInafiancaveis
      );
    });
...
function extrairValorMultaDoTexto(texto) {
  ...
  for (const regex of padroes) {
    ...
    const valorNormalizado = valorBruto.includes(",")
      ? valorBruto.replace(/\./g, "").split(",")[0]
      : valorBruto.replace(/\D/g, "");

    const numero = parseInt(valorNormalizado, 10);
...
function extrairValorMultaDaMensagem(msg) {
  const textoCompleto = extrairTextoCompleto(msg);
  return extrairValorMultaDoTexto(textoCompleto);
}

function contarInafiancaveisDaMensagem(msg, keywords) {
  const textoNormalizado = normalizarTexto(extrairTextoCompleto(msg));
  if (!textoNormalizado) return 0;

  const blocos = [];
  const regexSecaoCrimes =
    /(CRIMES(?:\/ARTIGOS)?|CRIME(?:S)?|MOTIVO)\s*([\s\S]{0,1200})/gi;
  let match;

  while ((match = regexSecaoCrimes.exec(textoNormalizado)) !== null) {
    blocos.push(match[2] || "");
  }

  const fonte = blocos.length > 0 ? blocos.join("\n") : textoNormalizado;
  const linhas = fonte
    .split(/\n|•|·|-/)
  ...
  linhas.forEach((linha) => {
    if (keywords.some((keyword) => linha.includes(keyword))) {
      total++;
    }
  });
