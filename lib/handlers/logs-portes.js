...
async function fetchMemberNickname(guildId, userId, botToken) {
  try {
...
async function enriquecerComNicks(logs, guildId, botToken) {
  const ids = [...new Set(logs.map((l) => l.emitidoPorIdDiscord).filter((id) => id && id !== "N/A"))];
  const cache = new Map();
  const results = await Promise.allSettled(
    ids.map((id) =>
      fetchMemberNickname(guildId, id, botToken).then((nick) => ({ id, nick }))
    )
  );
  results.forEach((r) => {
    if (r.status === "fulfilled" && r.value.nick) {
      cache.set(r.value.id, r.value.nick);
    }
  });
  return logs.map((log) => {
    const id = log.emitidoPorIdDiscord;
    return {
      ...log,
      emitidoPor: cache.get(id) || log.emitidoPor,
    };
  });
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    logs = await enriquecerComNicks(logs, process.env.Discord_Guild_ID, botToken);
